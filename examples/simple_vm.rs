use std::{path::{Path, PathBuf}, fs::File, io::copy};
use firec::{
    Machine,
    config::{Config, network::Interface}};
...
fn kernel_url() -> hyper::Uri {
    format!("https://s3.amazonaws.com/spec.ccfc.min/img/quickstart_guide/{}/kernels/vmlinux.bin", std::env::consts::ARCH)
    .parse::<hyper::Uri>()
    .unwrap()
}
...
fn rootfs_url() -> hyper::Uri {
    format!("https://s3.amazonaws.com/spec.ccfc.min/ci-artifacts/disks/{}/ubuntu-18.04.ext4", std::env::consts::ARCH)
    .parse::<hyper::Uri>()
    .unwrap()
}
...
async fn fetch_url(url: hyper::Uri, target_path: PathBuf) {
    ...
    let response = client.get(url.to_string()).send().await.expect("Could not download file");
    let mut file = File::create(target_path).expect("Could not create file");

    copy(&mut response.bytes().await.expect("Could not get bytes file into the system").as_ref(), &mut file).expect("Could not copy file");
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fetch_url(rootfs_url(), PathBuf::from("./examples/simple_vm/rootfs.ext4")).await;
    fetch_url(kernel_url(), PathBuf::from("./examples/simple_vm/kernel.bin")).await;
